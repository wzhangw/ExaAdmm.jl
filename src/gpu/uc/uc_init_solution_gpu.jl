function uc_init_generator_kernel_one_level(n::Int, t::Int, gen_start::Int,
    ...
    g = div(I, t, RoundUp)
    if g <= n
        v[gen_start + 2*(I-1)] = 0.5*(pgmin[g] + pgmax[g])
        v[gen_start + 2*(I-1)+1] = 0.5*(qgmin[g] + qgmax[g])
    end

    return
end

function uc_init_branch_bus_kernel_one_level(n::Int, t::Int, line_start::Int, rho_va::Float64,
    ...
    I = threadIdx().x + (blockDim().x * (blockIdx().x - 1))
    l = div(I, t, RoundUp)
    tidx = Base.mod(I, t) == 0 ? t : Base.mod(I, t)
    ...
    if l <= n
        ...
        pijt_idx = line_start + 8*(l-1)*t + 8*(tidx-1)
        u[pijt_idx] = YffR[l] * wij0 + YftR[l] * wR0
        u[pijt_idx+1] = -YffI[l] * wij0 - YftI[l] * wR0
        u[pijt_idx+2] = YttR[l] * wji0 + YtfR[l] * wR0
        u[pijt_idx+3] = -YttI[l] * wji0 - YtfI[l] * wR0
        ...
        rho[pijt_idx+4:pijt_idx+7] .= rho_va
    end
...
function init_solution!(
    model::UCModel{Float64,CuArray{Float64,1},CuArray{Int,1},CuArray{Float64,2}},
    ...
    @cuda threads=64 blocks=(div(model.ngen*model.t-1,64)+1) uc_init_generator_kernel_one_level(model.ngen, model.t, model.gen_start,
                    model.pgmax, model.pgmin, model.qgmax, model.qgmin, sol.v_curr)
    @cuda threads=64 blocks=(div(model.nline*model.t-1,64)+1) uc_init_branch_bus_kernel_one_level(model.nline, model.t, model.line_start, rho_va,
