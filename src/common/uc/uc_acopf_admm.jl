function solve_uc_acopf(case::String;
    ...
    mod = UCModel{T,TD,TI,TM}(env)

    if use_gpu
        # Set rateA in membuf.
        CUDA.@sync @cuda threads=64 blocks=(div(mod.nline*mod.t-1, 64)+1) uc_set_rateA_kernel(mod.nline, mod.t, mod.membuf, mod.rateA)
    else
        mod.membuf[29,:] .= mod.rateA
    end

    uc_admm_restart(env, mod)
    return env, mod
end